...
H_34_compatible = H_12[:576, :]
syn_34_compatible = syn_12[:576]

llr = np.array([1.75 if b == 0 else -1.75 for b in bob_blk])
L = np.round(llr * 4).astype(int)

M, N_cols = H_34_compatible.shape
V2C = np.zeros((M, N_cols), dtype=int)
C2V = np.zeros((M, N_cols), dtype=int)

for i in range(M):
    for j in range(N_cols):
        if H_34_compatible[i, j] == 1:
            V2C[i, j] = L[j]

MAX_ITER = 32
for it in range(MAX_ITER):
    for i in range(M):
        connected_vns = np.where(H_34_compatible[i, :] == 1)[0]
        for j in connected_vns:
            ...
            if syn_34_compatible[i] == 1:
                sgn = -sgn
            C2V[i, j] = sgn * min_val
            
    L_new = L.copy()
    for j in range(N_cols):
        connected_cns = np.where(H_34_compatible[:, j] == 1)[0]
